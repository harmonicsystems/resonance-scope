...
- **No build step, no bundler.** Vanilla HTML/CSS/JS (Web Audio + Canvas), hand-written, framework-free. The resonance instrument is `index.html`; the pitch detector is extracted into a plain classic `f0.js` (`window.F0.detect`/`noteName`) shared with the second page, `reading.html` (a simplified reading-passage / speaking-pitch view). Keep it this way — prefer adding a plain `.js`/`.css` file over a bundler. **One dependency exception:** the optional camera instrument needs robust lip landmarks, which realistically require MediaPipe FaceLandmarker. It is **vendored, not bundled** — committed static assets under `vendor/mediapipe/` (the `vision_bundle.mjs` ES module + `wasm/`) and `models/face_landmarker.task`, loaded via a local ES-module import and `FilesetResolver.forVisionTasks("./vendor/mediapipe/wasm")`. No CDN, no package manager, no build step. Do not add further dependencies without the same bar (real need, vendored locally, runs on-device).
...
**Longitudinal view = reproducibility over time (calibration-gated).** The per-point `t` timestamp turns the point set into a time series. `sessionize()` groups points into practice **sessions** by a 30-min gap (`SESSION_GAP_MS`; ≥30 min between freezes starts a new one — timezone-free, matches real sittings). The "Reproducibility over sessions" card trends **per-vowel cluster spread** across sessions (the same quantity `renderClusters` computes, just windowed) as compact sparklines — *is your production tightening?*. **The cross-session trend requires calibration**: uncalibrated spread is raw Hz (shifts with mic distance → not comparable across sessions), so uncalibrated users get a **locked** state, not a misleading trend — calibration earns the progress view the same way it earns VSA/FCR. Honesty guardrails are load-bearing: no trend verb until ≥3 sessions, neutral (never green/red), noise-aware ("holding steady"); thin sessions (<5 usable points) render grayed; `fb` low-confidence points are excluded-but-counted; a per-session F0-drift note cautions when a tighter cluster may just be pitch. **Export** is a self-describing local JSON (`{schema, schemaVersion, exportedAt, fields, calibration, points}`) with the **raw-Hz points verbatim** — never normalized — so it round-trips the store-raw invariant. Import, VSA/FCR trends, and recalibration-discontinuity markers are deferred.

**Reading view (`reading.html`) = connected-speech pitch, a second use case.** The main tool measures the *filter* from sustained vowels; the reading page measures the *source* over connected speech. Pick a phonetically-oriented passage (Rainbow / Bread / Dragons / Moon — embedded verbatim as static text, credited to `dnslp.github.io/sample`), read aloud, and it shows live F0 + a scrolling pitch contour, then on Stop a descriptive summary: mean speaking F0, pitch **range in semitones** (5th–95th percentile — robust to octave slips), pitch **variability** (SD in semitones — expressive vs. monotone), % voiced, duration. **On-charter:** the passage is only an *elicitation prompt* to get natural speech; we measure pitch (the source), never the words — no pronunciation grading, descriptive not diagnostic. Detection is the shared `F0.detect` (f0.js); smoothing is lighter here (connected speech moves faster). v1 is **ephemeral** (no storage/trend yet — that would reuse the session-view pattern). The two pages cross-link.
...
These prose limits notes are **kept alongside** a crisp **glossary** (a collapsed `<details>` above the footer) that defines every term/acronym (F0, F1/F2, VSA, FCR, cluster spread, aperture, cents, …). The glossary is rendered from a single `DEFINITIONS` array (`{group, term, color, def}`), color-coded to the tokens so it doubles as the legend — and it's the intended **single source of truth for a future in-context tooltip layer** (keep it data-driven; don't hand-write duplicate copy).
...
- Not multi-speaker, not real-time formant *tracking* (only freeze-frame), no recording/export of audio or video (by design — see on-device invariant). A local **JSON export of the derived points** (raw Hz + calibration) *is* available — it's a user-initiated `Blob` download of numbers only, never audio/video, and never a network upload.
...
5. ~~**Data export + session/time progress view** — local JSON export; per-vowel cluster-spread trend across practice sessions.~~ **Done** (import + VSA/FCR trends deferred).
6. **LPC formant tracking** to replace peak-picking when accuracy matters — the biggest accuracy lever now; normalization fixed the *scale*, LPC would fix the *estimate*.
...
- Repo layout: `index.html` (resonance instrument) + `reading.html` (reading/pitch view) + `f0.js` (shared pitch engine) + vendored `vendor/mediapipe/` (~19 MB WASM + ES module) + `models/face_landmarker.task` (~3.7 MB). All committed static assets — no install, no package manager, no build, no CI. `reading.html` uses no MediaPipe, so it also works from `file://` (`f0.js` is a classic script, not a module). Serves on Pages at `/resonance-scope/reading.html`. A plain HTML/JS linter is enough if a static check is ever wanted.
